...
Интеграционные тесты отправляют запросы к запущенному приложению по адресу
`http://127.0.0.1:8000`. Перед их запуском поднимите приложение и базу данных в
фоновом режиме:
...
Дождитесь статуса `healthy` у сервисов `app` и `db`. Сервис `migrate` должен
завершиться с кодом `0`.

Для локального запуска тестов потребуются Python 3.14 и
[uv](https://docs.astral.sh/uv/). Установите основные и тестовые зависимости,
после чего запустите pytest:
...
Запуск только интеграционных тестов кошельков:

```bash
uv run pytest src/test/test_wallet.py
```

Во время интеграционных тестов оставьте `APP_PORT=8000`, поскольку сейчас их
базовый URL использует порт `8000`.
